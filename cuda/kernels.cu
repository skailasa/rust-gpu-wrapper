...
extern "C" cudaError_t cudaMallocGeneric(void **data_d, int size_in_bytes);
...
extern "C" cudaError_t cudaMemcpyGenericDeviceToHost(void** data, void **data_d, int size_in_bytes)

extern "C" cudaError_t cudaMallocGeneric(void **data_d, int size_in_bytes) {
    cudaError_t status = cudaMalloc(data_d, size_in_bytes);
    return status;
}
